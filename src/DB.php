...
class DB extends \PDO
{
    ...
    function selectWhere($table, array $fields = null, array $conditions): array
    {
    ...
    //Función selectAllWithJoin
    function selectAllWithJoin($table1, $table2, array $fields = null, $join1, $join2): array
    {
        ...
        $sql = "SELECT {$columns} FROM {$table1} INNER JOIN {$table2}";
    ...
    function update(String $table, array $data, array $conditions)
    {
        if ($data) {
            ...
            $sql = "UPDATE {$table} SET modify_date = now() WHERE {$cond}";
    ...
    //Función remove
    function remove($tbl, $id, $post =null)
    {
    ...
    //Función removepost
    function removep($tbl, $id)
    {

        $sql = "DELETE FROM {$tbl} WHERE post = $id";
